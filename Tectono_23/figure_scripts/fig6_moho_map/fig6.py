...
set lajo = ~/Documents/ScientificColourMaps7/lajolla/lajolla.cpt
set bilbao = ~/Documents/ScientificColourMaps7/bilbao/bilbao.cpt
set devon = ~/Documents/ScientificColourMaps7/davos/davos.cpt
...
##############
gmt6 makecpt -C$lajo -T25/60/5 -D  > lajolla.cpt
gmt6 makecpt -Croma.cpt -T25/55/2.5 -I > devon_m.cpt
...
# gmt6 psxy ~/Research/Lake_eyre_data/geological_ft/Gawler_Final.gmt $Rmap $J -W0.74p,navy,- -O -K -V >> $PS ### crustal boundary

# gmt6 psxy $tect_arch $Rmap $J -W0.25p,firebrick -GMistyRose@70 -V -O -K  >> $PS # archean
# gmt6 psxy $tect_curna $Rmap $J -W0.25p,darkcyan -GAzure@40  -O -K  >> $PS # proterozoic musgrave
# gmt6 psxy $tect_musgrave $Rmap $J -W0.25p,darkcyan -GAzure@40 -O -K  >> $PS # proterozoic musgrave

# gmt6 psxy $sedi_officer $Rmap $J -W0.5p,plum -GLavenderBlush -V -O -K  >> $PS
# gmt6 psxy $sedi_LE $Rmap $J -W0.25p,DarkGoldenrod -GLightGoldenrodYellow -O -K  >> $PS
# gmt6 psxy $tect_THZ $Rmap $J -W0.25p,DarkGreen -GYellowGreen -O -K  >> $PS # torres hinge
# gmt6 psxy $tect_curna $Rmap $J -W0.25p,darkcyan -O -K  >> $PS # proterozoic musgrave
# gmt6 psxy $GAB $Rmap $J -W0.25p,SteelBlue -V -O -K  >> $PS # great artesian basin

# awk '{if ($1==0 || $1 > 130 || $1 < 145) print $0}' $rivers | gmt6 psxy  $Rmap $J -W0.025p,SteelBlue -V -O -K  >> $PS # great artesian basin
...
gmt6 psxy ~/Research/Lake_eyre_data/geological_ft/CrustalBoundaries.txt $Rmap $J -W0.5p,darkgreen,- -O -K >> $PS ### crustal boundary

# gmt6 psxy $basement $Rmap $J -W0.15p,darkred -O -K >> $PS ### crustal boundary
gmt6 psxy $tect_inl $Rmap $J -W0.15p,SaddleBrown -Gwhite@80 -O -K >> $PS ### crustal boundary



# gmt6 psxy ~/Research/south_aus/AU_stations_SA.txt -Ss.2 -Gblack $J $Rmap -O -K >> $PS
gmt6 psxy ~/Research/Lake_eyre_data/station/stations_all_BB.txt -: -W.08 -Sc.35 -Gwhite $J $Rmap -O -K >> $PS
gmt6 psxy ~/Research/Lake_eyre_data/station/stations_all_SP.txt -: -W.08 -Sc.35 -Gwhite $J $Rmap -O -K >> $PS

awk '{print $1,$2,($3-$4)*8.9}' stations_all_BB_Pms.txt > temp.txt
gmt6 psxy temp.txt -: -W.08 -Sc.35 -Cdevon_m.cpt $J $Rmap -O -K >> $PS

awk '{print $1,$2,($3-$4)*8.9}' stations_all_SP_Pms.txt > temp1.txt
gmt6 psxy temp1.txt -: -W.08 -Sc.35 -Cdevon_m.cpt $J $Rmap -O -K >> $PS

awk '{print $1,$2,($3-$4)*8.9}' stations_all_Pms_2.txt > temp2.txt
gmt6 psxy temp2.txt -: -W.08 -Sc.35 -V -Cdevon_m.cpt $J $Rmap -O -K >> $PS # plots higher pms value

awk '{print $1,$2,($3-$4)*8.9}' stations_all_Pms_1.txt > temp3.txt
gmt6 psxy temp3.txt -: -W.08 -Sc.21 -V -Cdevon_m.cpt $J $Rmap -O -K >> $PS # plots lower pms value

awk '{print $1,$2,($3-$4)*8.9}' 6k_stations_Ps-dt.txt > temp_sa.txt
awk '{print $1,$2,$4*8.9}' 6k_stations_Ps-dt.txt | gmt6 psxy -W.08 -Cdevon_m.cpt -Sc.35  $J $Rmap -O -V -K >> $PS
awk '{print $1,$2,$4*8.9}' 6k_stations_Ps.txt | gmt6 psxy -W.08 -Cdevon_m.cpt -Sc.35  $J $Rmap -O -V -K >> $PS
...
# awk '{print $2,$1,$3}' ~/Research/AuSREM-CM-moho/H-Mfiles/H-aitken.xyz | gmt6 psxy -W.01 -Ss.18 -Clajolla.cpt $J $Rmap -O -K >> $PS
############### Active
...
gmt6 pscoast $Rmap $J -Bx -By -Na/.05p -A10 -P -SLightSteelBlue  -Di -O -K -W.1p >> $PS #-A10+l -Ia
...
awk '{print $2,$1-.23,$3}' ~/Research/Lake_eyre_data/station/stations_all_*.txt | gmt6 pstext $Rmap $J -F+f3p,Helvetica-Bold -Gwhite -O -P -K >> $PS

# gmt6 psimage fig_inset.eps -Dx.1i/.1i+jTR+w.1i -V -O -P >> $PS
...
gmt6 pslegend -Dx1.5c/.75c+w2c/1.3.c+o-1c/-.5c -F+gwhite+p.1 -O $J $Rmap << EOF >> $PS
S 0.2c c 0.35c goldenrod - 0.2i 5G-6K
S 0.2c s 0.3c cadetblue - 0.2i Passive
S 0.2c t 0.3c maroon  - 0.2i Active
