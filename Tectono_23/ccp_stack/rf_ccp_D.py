...
prof_a=['AEB10','AEB06','AEB05','AEB03','AEB02','AES17','AES18','AES19','AES07','AEB07','AES20']#
...
prof_d_small_inlier=['AES16','AES09','AES04','AEB17','AES11','AES10','AEB04','AES14','AES15','AEB16']

prof_d1=['DOGFM','DOGFC','MOBEL','WYNBR','BULGU','MCDOU','INGOM','MTEBA','TWINS','WILGE','WHYML']# add these two later 'COMWH','WILGE',
prof_d2=['AES04','AEB17','AES11','AES10','AEB04','AES14','AES15','AEB16','AES06','AEB15','AES12']
prof_d3=['AEB12','AEB11','AEB08','AEB09','AEB07']
# 6K stations with sedi:  INGOM, MILCK, MTEBA, WHYML, WILGE, WIRRE
# 6K stations with no results: COMWH, YALYM, MERNA, NILPI,
prof_d_big=prof_d1+prof_d2+prof_d3

# inventory = client_5g.get_stations(network="5G",station='AEB07,AEB08,AEB09,AEB11,AEB13,AEB12',level='response')
inventory = client_5g.get_stations(network="5G",station=','.join(prof_d2+prof_d3),level='response')
#
rf_all=RFStream()
# station_name=['AES01']
for station_5G in prof_d3+prof_d2:
...
##
print('{} 5G stations done'.format(len(prof_d3+prof_d2)))
### aus array stations
#
client_sa = Client('http://auspass.edu.au:8080',user='6k',password='melon93')
inventory_sa = client_sa.get_stations(network='6K', station=','.join(prof_d1),level='response')

for station_6K in prof_d1:
    try:
    ...
    #############################################################
        ...
        print('Data/rf not found for station',station_6K)
##
print('Len of all rfs',len(prof_d_big))
###
# rf_all.write('rfs_DD_long.H5', 'H5')
print('Len of rf_all=',len(rf_all)) #4285

sys.exit()
############ CCP bit
#kw = {'tt_model': 'LA_moho35'}
## PLOTTING D, 135 -28.7; 137 -27.9 # 215km #66.04 azimuth # SMALL D AROUND INLIER
## PLOTTING D, 132.5 -30.5; 140.4 -26.6 # 885km #62.6 azimuth # long D

rf_all=read_rf('rfs_DD_long.H5', 'H5')
print('############')
print('Doing Profile D')
stream=rf_all.copy()
...
boxes = get_profile_boxes((-30.5, 132.5), 62.6, np.linspace(0, 900, 181), width=260)
...
plt.box(False)
plt.savefig('pp_50km_DD_long.pdf',bbox_inches='tight', pad_inches=0.1)
plt.show()

sys.exit()
pstream = profile(tqdm(stream), boxes)
pstream.write('profile_stream_DD_50km.H5', 'H5')
print(pstream)
#####
...
### change histogram limit manually in fx_ccp.py line #190
fx.plot_profile(pstream_temp.normalize(),scale=4,figsize=(20, 5.5), top='hist',fillcolors=('indianred', 'lightsteelblue'))#,moveout_model='ak135')
plt.gcf()#.set_size_inches(50, 10)
plt.savefig('ccp_50km_DD_long_>4rfs.pdf',bbox_inches='tight', pad_inches=0.1)
plt.show()
